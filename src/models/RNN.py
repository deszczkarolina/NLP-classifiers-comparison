...
def train(train_dataset, test_dataset, classes_num):
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(config.RNN['parameters']['LSTM_OUTPUT_DIM'], name='LSTM')),
        tf.keras.layers.Dense(config.RNN['parameters']['DENSE_LAYER_OUTPUT_DIM'],
                              activation=config.RNN['parameters']['DENSE_LAYER_ACTIVATION'], name='Dense'),
        tf.keras.layers.Dense(classes_num, name='Classifier')
    ])

    model.compile(loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
                  optimizer=tf.keras.optimizers.Adam(config.RNN['parameters']["LEARNING_RATE"]),
                  metrics=['accuracy'])
